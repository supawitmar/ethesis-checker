 ...
 *
 * จึงสร้างวันที่ "ในเขตเวลาของสเปรดชีต" ตรง ๆ ด้วย Utilities.parseDate แล้วตั้งเป็น
 * เที่ยงวันเผื่อการปัดเศษ — ไม่ใช่หวังว่าสองเขตเวลาจะห่างกันไม่เกินครึ่งวัน
 * (เขตเวลาของ Google มีตั้งแต่ GMT-11 ถึง GMT+14 ห่างกันได้ถึง 25 ชั่วโมง)
 */
function toDate_(iso) {
  ...
  try {
    return Utilities.parseDate(day + ' 12:00:00', sheetTimeZone_(), 'yyyy-MM-dd HH:mm:ss');
  } catch (err) {
    // กันพังไว้ — อย่างน้อยยังได้เที่ยงวันของเขตเวลาสคริปต์ ซึ่งคลาดได้ไม่เกินครึ่งวัน
