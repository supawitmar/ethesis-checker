 ...
 *
 * ช่อง "รายละเอียดที่ต้องแก้ไข" เป็นของเพิ่มทีหลัง ไม่บังคับ — เพิ่มหัวตารางชื่อนี้
 ...
 */
var HEADER_DETAILS = ['รายละเอียดที่ต้องแก้ไข', 'รายละเอียดที่ส่งให้แก้ไข',
                      'รายละเอียดที่ส่งให้นักศึกษาแก้ไข', 'รายละเอียด', 'Details'];
var HEADER_SCAN_ROWS = 10;              // หัวตารางอยู่ไม่เกินสิบแถวแรก (มีแถวสถิติอยู่บน)

/**
 * ความสูงแถวหลังเขียนรายละเอียด (พิกเซล) — 21 คือความสูงมาตรฐานของ Google Sheets
 *
 * ข้อความที่ส่งให้นักศึกษามีการขึ้นบรรทัดใหม่อยู่ข้างในจริง ๆ (ข้อ 1. 2. 3. ...)
 * "ตัดส่วนเกิน" (CLIP) กันได้แค่ข้อความที่ยาวเกินความกว้างของช่อง แต่ไม่กันบรรทัดใหม่
 * แถวจึงยืดสูงเป็นสิบกว่าบรรทัดตามจำนวนข้อ (เจ้าหน้าที่แจ้ง ก.ย. 2569 "ไม่อยากให้มันขยาย")
 * ต้องสั่งความสูงแถวกลับทุกครั้งที่เขียน ข้อความยังอยู่ครบ กดที่ช่องแล้วอ่านได้ในแถบสูตร
 * ถ้าอยากให้สูงกว่านี้ แก้ตัวเลขบรรทัดนี้บรรทัดเดียว
 */
var DETAILS_ROW_HEIGHT = 21;
  ...
  try {
    ...
    if (body.details) {
      ...
      if (detailsAt) {
        ...
        clip_(detailsCell);
        keepRowHeight_(sheet, row);   // ไม่ให้แถวยืดตามจำนวนบรรทัดของข้อความ
      } else {
        missing.push('รายละเอียดที่ต้องแก้ไข');   // ต้องบอก ไม่ใช่ทิ้งข้อความหายเงียบ ๆ
      }
    }
