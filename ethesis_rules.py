...
RULE_CATALOG = {
    ...
    "PAGE.NUMBERING_SEQUENCE": {
        "title": "ความต่อเนื่องของเลขหน้า (ส่วนนำและเนื้อหา)",
        "basis": "ประกาศฯ คู่มือ และกติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "ประกาศฯ ข้อ 4.1.1.5 หน้า 4",
            "คู่มือ หน้า 6, 16-21",
            "กติกาปฏิบัติงาน (ก.ย. 2569): เจ้าหน้าที่กำหนดให้เป็นสีส้ม",
        ),
        # แยกจาก PAGE.NUMBERING เพราะกฎนั้นคุมหลายเรื่องที่ยังเป็นสีแดงอยู่
        # (ชนิดเลขหน้าผิด / ไม่มีเลขหน้า / เลขหน้าอารบิกไม่เริ่มที่บทที่ 1)
        "failure_zone": "ORANGE",
    },
    ...
    "FORM.LATE_FEE_NOTE": {
        "title": "ข้อความปิดท้าย: ค่าปรับส่งล่าช้า วิธีส่งกลับ และช่องทางติดต่อ",
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "กติกาปฏิบัติงาน (ก.ย. 2569): ระบบไม่รู้เรื่องค่าปรับ เจ้าหน้าที่เลือกเอง "
            "ว่านักศึกษามีค่าปรับหรือไม่ แล้วข้อความปิดท้ายชุดที่กำหนดไว้จะต่อท้าย "
            "ข้อความสรุป ไม่นับเป็นจุดที่ต้องแก้",
        ),
    },
    "FRONT.SIGNATURE_LAYOUT": {
        "title": "โครงสร้างหน้าลงนาม (กรอบตาราง ลำดับการวางชื่อ ฟอนต์ ช่องที่ต้องถมขาว)",
        "basis": "คู่มือและกติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "คู่มือ หน้า 11-12 (หน้าลงนามสองหน้าแรกของส่วนนำ) "
            "และลูกศรสีเหลืองในคู่มือที่แสดงลำดับการวางชื่อกรรมการลงในตาราง",
            "กติกาปฏิบัติงาน (ก.ย. 2569): ระบบอ่านโครงสร้างของหน้านี้ไม่ได้ "
            "เจ้าหน้าที่เป็นผู้ตัดสินแล้วกดเพิ่มถ้อยคำชุดที่กำหนดไว้เข้าข้อความสรุป",
        ),
    },
    "FRONT.STRUCTURE": {
    ...
    "FORM.BOOK_LANGUAGE": {
        "title": "ภาษาของเล่มต้องตรงกับหลักสูตรที่ได้รับอนุมัติ",
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่ และ template ที่แยกตามภาษาของเล่ม",
        "references": (
            "กติกาปฏิบัติงาน (ก.ย. 2569): ถ้าภาษาของเล่มไม่ตรงกับที่ได้รับอนุมัติ "
            "ให้หยุดตรวจส่วนอื่นทั้งหมด แล้วแจ้งจุดผิดข้อเดียวว่าเล่มผิดภาษา "
            "และให้ข้อนี้อยู่บนสุดของรายงาน",
            "Electronic File template-2026: หน้าปกและหน้าลงนามของเล่มไทยกับเล่มอังกฤษ "
            "ใช้ข้อความบังคับคนละชุดกัน",
            "ข้อมูลอนุมัติจาก eThesis/บฑ.1 (ช่องหลักสูตร และบรรทัด \"ภาษาที่เขียน\")",
        ),
    },
    "FORM.DOC_TYPE": {
        "title": "ประเภทเล่มที่เล่มเขียนไว้ต้องตรงกับที่ได้รับอนุมัติ",
        "basis": "ข้อมูลอนุมัติ template ทางการ และกติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "กติกาปฏิบัติงาน (ก.ย. 2569): eThesis อนุมัติเป็นสารนิพนธ์ แต่เล่มเขียน "
            "INDEPENDENT STUDY \"ให้ฟ้องด้วย\" — ประเภทที่อนุมัติยึดตามไฟล์ eThesis "
            "ก่อนช่องประเภทเล่มที่เลือกบนหน้าอัปโหลด",
            "Electronic File template-2026: คำระบุประเภทเล่มบนหน้าปก หน้าลงนาม "
            "และบรรทัดคณะกรรมการที่ปรึกษาในบทคัดย่อ",
            "ข้อมูลอนุมัติจาก eThesis (แผนการศึกษา: วิทยานิพนธ์ / สารนิพนธ์ / การค้นคว้าอิสระ)",
        ),
    },
    "FRONT.TITLE_ONE_LANGUAGE": {
        "title": "หน้าปกและหน้าลงนามต้องมีชื่อเรื่องภาษาเดียว",
        "basis": "template ทางการแยกตามภาษาของเล่ม และกติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "ประกาศฯ ข้อ 4.1.1.1-4.1.1.2 หน้า 3-4",
            "Electronic File template-2026: หน้าปกและหน้าลงนามของแต่ละภาษามีช่อง"
            "ชื่อเรื่องช่องเดียว",
            "กติกาปฏิบัติงาน (ก.ย. 2569): เล่มภาษาไทยให้มีเฉพาะชื่อเรื่องภาษาไทย "
            "เล่มภาษาอังกฤษให้มีเฉพาะชื่อเรื่องภาษาอังกฤษ และต้องเทียบกับชื่อเรื่อง"
            "ที่ดึงจากข้อมูลระบบเท่านั้น",
            "ข้อมูลอนุมัติจาก eThesis/บฑ.1 (ช่องชื่อหัวข้อภาษาไทย และภาษาอังกฤษ)",
        ),
    },
    ...
    "FRONT.KEYWORD_COUNT": {
        "title": "จำนวน keyword ท้ายบทคัดย่อ",
        "basis": "ประกาศฯ และกติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "ประกาศฯ ข้อ 4.1.1.7 หน้า 5",
            "กติกาปฏิบัติงาน (ก.ย. 2569): เกิน 5 คำได้ ให้เป็นข้อสังเกตสีเหลือง",
        ),
        # แยกจาก FRONT.ABSTRACT เพราะกฎนั้นคุมอีกหลายเรื่องที่ยังเป็นสีแดง
        "failure_zone": "YELLOW",
    },
    "FORM.FONT_UNREADABLE": {
        "title": "รหัส/ชื่อนักศึกษาที่ระบบอ่านไม่ออกเพราะฟอนต์ในไฟล์",
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "กติกาปฏิบัติงาน (ก.ย. 2569): ฟอนต์ที่ฝังในไฟล์ทำให้ตัวเลขหรือชื่อเพี้ยน "
            "เป็นสีส้ม ให้เจ้าหน้าที่เปิดหน้าจริงดูแล้วกดยืนยัน",
        ),
        "failure_zone": "ORANGE",
    },
    "BODY.CHAPTER_NUMBER": {
        "title": "เลขบทในหัวบทต้องเป็นเลขอารบิก",
        "basis": "template และกติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "คู่มือ หน้า 7, 16, 18 และ 83-93",
            "กติกาปฏิบัติงาน (ก.ย. 2569): เลขอารบิกทั้งเล่มไทยและเล่มอังกฤษ "
            "เล่มไทยใช้เลขอารบิก ไม่ใช่เลขไทย",
        ),
    },
    "BODY.OPTION2": {
