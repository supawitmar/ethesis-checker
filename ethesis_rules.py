...
RULE_CATALOG = {
    ...
    "PAGE.SIGNATURE_LABEL": {
        "title": "เลขหน้าของหน้าลงนามต้องเป็น ก/ข (ไทย) หรือ i/ii (อังกฤษ)",
        "basis": "ประกาศฯ คู่มือ และกติกาปฏิบัติงานของเจ้าหน้าที่",
        "failure_zone": "ORANGE",
        "references": (
            "ประกาศฯ ข้อ 4.1.1.5 หน้า 4 (การลำดับเลขหน้าส่วนนำ)",
            "คู่มือ หน้า 11-12 (หน้าลงนามสองหน้าแรกของส่วนนำ)",
            "กติกาปฏิบัติงาน (ส.ค. 2569): เลขหน้าลงนามที่ผิดทำให้การเรียงเลขหน้า"
            "หน้าอื่นผิดตามไปด้วย จึงแจ้งเป็นสีส้มให้เจ้าหน้าที่ยืนยัน "
            "ไม่ฟ้องแดงซ้ำกับข้อ 'เลขหน้าไม่ต่อเนื่อง' ที่เกิดจากต้นเหตุเดียวกัน",
        ),
    },
    "FRONT.STRUCTURE": {
    ...
    "FRONT.COVER": {
        # ข้อมูลที่ต้องตรงข้อมูลอนุมัติ (ชื่อเรื่อง/ชื่อผู้เขียน/ปริญญา) อยู่ที่
        # FORM.APPROVED_MATCH — รหัสนี้ใช้กับ "รูปแบบเฉพาะของหน้าปก" เท่านั้น
        "title": "รูปแบบเฉพาะของหน้าปก (บรรทัดปี / ห้ามมีรหัสนักศึกษา)",
    ...
    "FRONT.COVER_FIRST": {
        "title": "หน้าปกต้องเป็นแผ่นแรกของไฟล์",
        "basis": "ประกาศฯ คู่มือ และกติกาปฏิบัติงานของเจ้าหน้าที่",
        "references": (
            "ประกาศฯ ข้อ 4.1 หน้า 3 (ลำดับส่วนประกอบของเล่ม)",
            "กติกาปฏิบัติงาน (ก.ย. 2569): ลำดับของไฟล์คือ หน้าปก หน้าลงนาม 1 "
            "หน้าลงนาม 2 กิตติกรรมประกาศ บทคัดย่อ สารบัญ เนื้อหารายบท "
            "รายการอ้างอิง ภาคผนวก (ถ้ามี) และประวัติผู้วิจัย "
            "หน้าปกต้องอยู่แผ่นแรกเสมอ",
        ),
    },
    ...
    "FRONT.COMMITTEE": {
        "title": "รายชื่อและตำแหน่งกรรมการบนหน้าลงนาม",
        "basis": "ข้อมูลอนุมัติจาก eThesis/บฑ.1 และ บฑ.2 + คู่มือรูปแบบตารางลายเซ็น",
        "references": (
            "ข้อมูลอนุมัติจาก eThesis (คณะกรรมการที่ปรึกษา/คณะกรรมการสอบ)",
            "คู่มือ หน้า 11-12 และ template ส่วนนำ (ตารางลายเซ็น 2 คอลัมน์)",
        ),
    },
    ...
    "FRONT.ABSTRACT_COMMA": {
        "title": "จุลภาคคั่นชื่อกับคุณวุฒิในรายชื่อกรรมการ (ข้อสังเกต)",
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่",
        "failure_zone": "YELLOW",
        "references": (
            "กติกาปฏิบัติงาน (ส.ค. 2569): เครื่องหมายจุลภาคที่ขาดไปในรายชื่อกรรมการ "
            "ให้แจ้งเป็นข้อสังเกตสีเหลือง เล่มที่มีแต่ข้อนี้ผ่านได้",
            "คู่มือ หน้า 14-15 (รูปแบบ 'ชื่อ นามสกุล, คุณวุฒิ')",
        ),
    },
    "FRONT.TOC": {
    ...
    "FRONT.TOC_PAGE_REF": {
        "title": "เลขหน้าที่สารบัญอ้างถึง (ข้อสังเกต ไม่ใช่จุดตัดสิน)",
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่",
        "failure_zone": "YELLOW",
        "references": (
            "กติกาปฏิบัติงาน (ส.ค. 2569): เลิกตรวจเทียบเลขหน้าในสารบัญกับหน้าจริง "
            "และเลิกตรวจการเรียงเลขหน้าของรายการในสารบัญ ถ้าพบให้บันทึกเป็น"
            "ข้อสังเกตสีเหลือง เล่มที่มีแต่ข้อนี้ผ่านได้",
        ),
    },
    "FRONT.ORDER": {
    ...
    "FORMAT.ABSTRACT_BOLD": {
        ...
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่",
        "failure_zone": "YELLOW",
        "references": ("กติกาปฏิบัติงาน (ส.ค. 2569): หน้าบทคัดย่อไม่ควรมีตัวหนา "
                       "แต่พอรับได้ จึงเป็นข้อสังเกตสีเหลือง ผ่านได้",),
    },
    "FORMAT.ABSTRACT_LAYOUT": {
        "title": "การจัดวางชื่อเรื่องบนหน้าบทคัดย่อ",
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่ + รูปแบบที่พบในเล่มจริง",
        "failure_zone": "ORANGE",
        "references": ("กติกาปฏิบัติงาน (ส.ค. 2569): ชื่อเรื่องต้องชิดซ้าย "
                       "ไม่ใช่กึ่งกลางหรือชิดขวา",
                       "เล่มจริง 10 เล่ม: 8 เล่มพิมพ์ชิดซ้าย"),
    },
    "PAGE.BLANK": {
    ...
    "PAGE.HEADER": {
        "title": "หัวกระดาษส่วนเนื้อหาและส่วนท้าย มีเพียงเลขหน้า",
        "basis": "ประกาศฯ และคู่มือ",
        "failure_zone": "ORANGE",
        "references": (
            "ประกาศฯ ข้อ 4.1.1.5 หน้า 4 (การวางเลขหน้า)",
            "คู่มือ หน้า 6, 16-21 (หัวกระดาษมีเฉพาะเลขหน้า ไม่มี running head)",
        ),
    },
    "UNCERTAIN.REVIEW": {
    ...
    "FORM.DEGREE_SPACING": {
        "title": "ชื่อปริญญาต่างจากข้อมูลอนุมัติเฉพาะวรรคตอน/ช่องว่าง (ข้อสังเกต)",
        "basis": "กติกาปฏิบัติงานของเจ้าหน้าที่",
        "failure_zone": "YELLOW",
        "references": (
            "กติกาปฏิบัติงาน (ส.ค. 2569): ชื่อปริญญาที่ตัวอักษรครบทุกตัว ต่างแค่ "
            "ช่องว่างหรือเครื่องหมายวรรคตอน เช่น \"M.Sc. ()\" กับ \"M.Sc.()\" "
            "ให้แจ้งเป็นข้อสังเกตสีเหลือง เล่มที่มีแต่ข้อนี้ผ่านได้",
        ),
    },
    "FORM.APPROVED_MATCH": {
...
BODY_RULES = {
    ...
    # เปิดไว้เพื่อ "บันทึกเป็นข้อสังเกต" เท่านั้น ไม่ใช่จุดตัดสินอีกแล้ว
    # (กติกา ส.ค. 2569: เลิกตรวจเลขหน้าที่สารบัญอ้างถึง → FRONT.TOC_PAGE_REF สีเหลือง)
...
# นโยบายเจ้าหน้าที่ล่าสุด: ยึด "ประกาศ" เป็นหลักเสมอ ชื่อบทที่ต่างจากประกาศแม้แต่
# ตัวเดียว (เช่น บทที่ 2 "LITERATURE REVIEW" เกิน S เป็น "LITERATURE REVIEWS")
# ถือเป็น "ชื่อบทพิมพ์ผิดเล็กน้อย" = แดง เหมือนบทอื่น ๆ ไม่ยกเว้นให้เป็นส้มอีก
# (เดิมเคยยกเว้นตามคู่มือ) — key คือ (option, chapter_no)
...
NOT_CHECKED = (
    ...
    "เนื้อหาของคุณวุฒิ (Degree/Subject) ใต้ชื่อกรรมการ "
    "(ระบบตรวจว่ามีบรรทัดคุณวุฒิครบทุกคน แต่ไม่ตรวจว่าคุณวุฒิถูกต้องหรือไม่)",
    "ชื่อ-สกุลและตัวสะกดของกรรมการ ทั้งบนหน้าลงนามและหน้าบทคัดย่อ "
    "(ระบบนับให้ว่าจำนวนครบตามฟอร์มต้นทางหรือไม่ แล้วพิมพ์รายชื่อตามฟอร์ม "
    "ไว้ในรายการสีม่วงให้เจ้าหน้าที่ทานเอง)",
    "ตำแหน่งการวางชื่อกรรมการในตาราง (แต่ละเล่มจัดหน้าไม่เหมือนกัน)",
    ...
    "เลขหน้าที่สารบัญอ้างถึงตรงกับหน้าจริงหรือไม่ "
    "(ถ้าพบว่าไม่ตรง ระบบบันทึกไว้เป็นข้อสังเกตสีเหลือง เล่มที่มีแต่ข้อนี้ผ่านได้)",
)
